...
@section('title', 'Edit Absensi - ADMA Absensi Kantor')
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="alert alert-info d-flex align-items-center">
                    ...
                    <div>
                        <strong>Edit absensi untuk:</strong> {{ $absensi->user->name }} - {{ $absensi->tanggal->format('d/m/Y') }}
                        <br>
                        <small class="text-muted">
                            ...
                            @elseif($absensi->jam_masuk || $absensi->jam_pulang)
                                <span class="badge bg-info">✏️ Manual Input</span>
                            @else
                                <span class="badge bg-secondary">📝 Bulk Input</span>
                            @endif
                        </small>
                    </div>
                </div>

                <form action="{{ route('admin.absensi.update', $absensi) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    @if($absensi->foto_pulang)
                                        <div class="col-md-6 text-center">
                                            <p><strong>Foto Pulang</strong></p>
                                            <img src="{{ asset('storage/' . $absensi->foto_pulang) }}" alt="Foto Pulang" 
                                                 class="img-thumbnail" style="max-height: 150px;">
                                        </div>
                                    @endif
                                </div>
                    ...
                    <div class="row" id="foto_fields">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label class="form-label">Ganti Foto Masuk</label>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label class="form-label">Ganti Foto Pulang</label>
                    ...
                    <!-- Additional Info -->
                    @if($absensi->is_within_geofence_masuk !== null || $absensi->distance_from_office_masuk !== null)
                        <div class="card bg-info bg-opacity-10 mb-3">
                            <div class="card-header">
                                <h6 class="mb-0">🗺️ Informasi Geofencing</h6>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <span class="badge bg-{{ $absensi->is_within_geofence_masuk ? 'success' : 'danger' }}">
                                            {{ $absensi->is_within_geofence_masuk ? 'Dalam Area' : 'Luar Area' }}
                                        </span>
                                        @if($absensi->distance_from_office_masuk)
                                            <br><small>Jarak: {{ $absensi->distance_from_office_masuk }}m</small>
                                        @endif
                                    </div>
                                    <div class="col-md-6">
                                        <strong>Status Pulang:</strong>
                                        <span class="badge bg-{{ $absensi->is_within_geofence_pulang ? 'success' : 'danger' }}">
                                            {{ $absensi->is_within_geofence_pulang ? 'Dalam Area' : 'Luar Area' }}
                                        </span>
                                        @if($absensi->distance_from_office_pulang)
                                            <br><small>Jarak: {{ $absensi->distance_from_office_pulang }}m</small>
                                        @endif
                                    </div>
                                </div>
                            </div>
